...
Function Get-ListingDiff {
	param(
		...
		$PRManifest = ($clip -split "`n" | Where-Object { $_ -match '.yaml' } | Where-Object { $_ -match $PackageIdentifier } | ForEach-Object { ($_ -split '/')[-1] -replace "$($PackageIdentifier)[.]", '' })
	)
	if ($CurrentManifest -ne 'Error') {
		Compare-Object $currentManifest $PRManifest
	} else {
		$CurrentManifest
	}
}
